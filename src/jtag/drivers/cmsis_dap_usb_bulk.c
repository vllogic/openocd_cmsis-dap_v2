/***************************************************************************
 *   Copyright (C) 2018 by Micka?l Thomas                                  *
 *   mickael9@gmail.com                                                    *
 *                                                                         *
	...
	for (int i = 0; i < num_devices; i++) {
		...
		for (int config = 0; config < dev_desc.bNumConfigurations; config++) {
			...
			for (int interface = 0; interface < config_desc->bNumInterfaces; interface++) {
				 ...
				 *  - Endpoint 1: Bulk Out ¨C used for commands received from host PC.
				 *  - Endpoint 2: Bulk In ¨C used for responses send to host PC.
				 *  - Endpoint 3: Bulk In (optional) ¨C used for streaming SWO trace (if enabled with SWO_STREAM).
				 */
