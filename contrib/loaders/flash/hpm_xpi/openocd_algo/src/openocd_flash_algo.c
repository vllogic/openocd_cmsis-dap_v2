...
__attribute__ ((section(".flash_algo.text"))) uint32_t flash_init(uint32_t flash_base,
										uint32_t header, uint32_t opt0, uint32_t opt1)
{
	uint32_t i = 0;
	XPI_Type *xpi_base;
...
__attribute__ ((section(".flash_algo.text"))) uint32_t flash_program(uint32_t flash_base,
												uint32_t address, uint32_t *buf, uint32_t size)
{
	XPI_Type *xpi_base;
	hpm_stat_t stat;
